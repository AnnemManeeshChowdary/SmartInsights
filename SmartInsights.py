    ...
    if len(numeric_cols) > 0:
        try:
            ...
            if len(numeric_cols) >= 2:
                ...
                col_means = df[numeric_cols].mean().replace(0, np.nan)
                cv_values = (df[numeric_cols].std() / col_means).abs()
                high_variance_cols = cv_values[cv_values > 1].dropna().index.tolist()
        ...
        if date_cols and numeric_cols:
            ...
            try:
                ...
                if date_range > 365*2:  # More than 2 years
                    grouper = pd.Grouper(key=date_col, freq='QE')
                    group_name = "Quarterly"
                elif date_range > 90:  # More than 3 months
                    grouper = pd.Grouper(key=date_col, freq='ME')
                    group_name = "Monthly"
...
def render_nlp_insights_page(df):
    ...
    # ── Category Bar
        ...
        _nlp_bar_n_cats = df[col].nunique() if col in df.columns else 10
        _nlp_bar_max = max(4, min(30, _nlp_bar_n_cats))
        _nlp_bar_default = min(10, _nlp_bar_max)
        _nlp_bar_min = min(3, _nlp_bar_max - 1)
        top_n = st.slider("Show top N categories", _nlp_bar_min, _nlp_bar_max, _nlp_bar_default, key="nlp_bar_n")
...
# ═══════════════════════════════════════════════════════════════════════════════
# SECTOR ANALYSIS — AUTO-DETECTION ENGINE
# ═══════════════════════════════════════════════════════════════════════════════

SECTOR_PROFILES = {
    "Finance / Banking": {
        "keywords": [
            "loan","credit","debit","bank","account","transaction","balance","interest",
            "deposit","withdrawal","mortgage","debt","equity","finance","payment","invoice",
            "revenue","profit","loss","asset","liability","portfolio","fund","capital",
            "expense","income","cost","budget","ledger","audit","tax","emi","nbfc",
            "npa","default","collateral","premium","claim","policy","insure","amount"
        ],
        "segment_keywords": [
            "loan_type","account_type","branch","region","product","customer_tier",
            "segment","category","type","class","status","grade","tier","zone","division"
        ],
        "metric_keywords": [
            "balance","amount","revenue","profit","interest","transaction","payment",
            "income","expense","cost","loss","return","yield","rate","fee","charge",
            "premium","claim","emi","principal","outstanding","overdue","default"
        ],
        "description": "Finance / Banking dataset detected — analyzing loan portfolios, account segments, transaction volumes, and risk metrics.",
        "guide": "Focus on balance concentration by segment and default/NPA rates. High variance in transaction amounts across branches often signals operational inefficiencies.",
        "agg": "Sum",
        "color": "#00BCD4"
    },
    "Stock / Investments": {
        "keywords": [
            "stock","share","equity","market","ticker","symbol","nse","bse","nasdaq",
            "nyse","price","open","close","high","low","volume","return","dividend",
            "pe_ratio","eps","market_cap","index","portfolio","trade","broker","sector",
            "52week","beta","alpha","volatility","momentum","rsi","macd","technical"
        ],
        "segment_keywords": [
            "sector","industry","exchange","asset_class","geography","market_cap","ticker",
            "symbol","company","stock","fund","etf","category","type","class"
        ],
        "metric_keywords": [
            "price","close","open","high","low","volume","return","change","pct_change",
            "market_cap","pe","eps","dividend","yield","beta","alpha","52week_high"
        ],
        "description": "Stock / Investment dataset detected — analyzing portfolio performance, sector allocation, and return metrics.",
        "guide": "Check return concentration by sector. High beta stocks amplify market moves — compare risk-adjusted returns, not raw returns.",
        "agg": "Mean",
        "color": "#4CAF50"
    },
    "Healthcare / Pharma": {
        "keywords": [
            "patient","hospital","doctor","drug","medicine","diagnosis","treatment",
            "disease","clinical","health","medical","pharmacy","prescription","dosage",
            "recovery","readmission","surgery","ward","bed","icu","nurse","physician",
            "insurance","claim","procedure","icd","cpt","lab","test","result","vital"
        ],
        "segment_keywords": [
            "department","ward","specialty","drug","diagnosis","condition","age_group",
            "insurance","gender","region","hospital","clinic","doctor","physician"
        ],
        "metric_keywords": [
            "cost","revenue","patients","count","recovery","readmission","length_of_stay",
            "satisfaction","rating","dosage","units","claims","amount","charges","visits"
        ],
        "description": "Healthcare / Pharma dataset detected — analyzing patient metrics, departmental performance, and cost efficiency.",
        "guide": "Cost-per-patient ratios and readmission rates are the key performance drivers. High variability between departments often signals staffing or resource issues.",
        "agg": "Mean",
        "color": "#FF5370"
    },
    "Retail / E-Commerce": {
        "keywords": [
            "product","sale","order","customer","store","sku","category","brand",
            "retail","ecommerce","shop","cart","purchase","return","refund","discount",
            "coupon","inventory","stock","shipment","delivery","channel","online","offline",
            "basket","unit","quantity","price","margin","revenue","profit","commission"
        ],
        "segment_keywords": [
            "category","product","brand","store","region","channel","customer_segment",
            "sub_category","department","segment","type","city","state","country"
        ],
        "metric_keywords": [
            "sales","revenue","profit","units","quantity","amount","discount","margin",
            "orders","returns","price","cost","basket_size","commission","shipping"
        ],
        "description": "Retail / E-Commerce dataset detected — analyzing product sales, category performance, and store metrics.",
        "guide": "Watch for category concentration risk. If top 3 products drive 70%+ of revenue, business is fragile. Compare online vs offline channel margins.",
        "agg": "Sum",
        "color": "#FF9800"
    },
    "Manufacturing / Supply Chain": {
        "keywords": [
            "production","plant","factory","supplier","machine","defect","quality",
            "output","yield","downtime","shift","line","batch","sku","warehouse",
            "inventory","lead_time","cycle_time","oee","reject","scrap","rework",
            "logistics","freight","shipping","vendor","procurement","raw_material"
        ],
        "segment_keywords": [
            "plant","line","shift","supplier","product_line","machine","region",
            "department","category","vendor","warehouse","zone","country"
        ],
        "metric_keywords": [
            "output","defects","cost","downtime","lead_time","yield","quality",
            "units","quantity","oee","reject","scrap","freight","inventory","cycle_time"
        ],
        "description": "Manufacturing / Supply Chain dataset detected — analyzing production output, defect rates, and supplier performance.",
        "guide": "Small defect rate differences compound at scale. Benchmark each line against the plant average — outliers need root cause analysis.",
        "agg": "Sum",
        "color": "#9C27B0"
    },
    "Education": {
        "keywords": [
            "student","course","grade","score","exam","marks","attendance","pass","fail",
            "enrollment","faculty","department","subject","college","university","school",
            "semester","cgpa","gpa","result","scholarship","dropout","graduation","class"
        ],
        "segment_keywords": [
            "department","course","subject","grade_level","campus","cohort","faculty",
            "program","stream","batch","section","school","college","semester"
        ],
        "metric_keywords": [
            "score","marks","grade","attendance","pass_rate","enrollment","cgpa","gpa",
            "graduation_rate","dropout","scholarship","count","average","percentage"
        ],
        "description": "Education dataset detected — analyzing student performance, course outcomes, and departmental metrics.",
        "guide": "Look for performance gaps between departments and cohorts. Low attendance usually predicts poor outcomes — flag those segments early.",
        "agg": "Mean",
        "color": "#2196F3"
    },
    "HR / Workforce": {
        "keywords": [
            "employee","staff","salary","department","hire","attrition","leave","absent",
            "performance","appraisal","promotion","designation","role","headcount","payroll",
            "bonus","increment","resignation","termination","joining","experience","tenure"
        ],
        "segment_keywords": [
            "department","designation","role","gender","location","grade","level",
            "team","division","band","category","experience_band","tenure_band"
        ],
        "metric_keywords": [
            "salary","bonus","headcount","attrition","performance","rating","leaves",
            "absent","increment","experience","tenure","count","average","total"
        ],
        "description": "HR / Workforce dataset detected — analyzing employee performance, attrition, salary bands, and department metrics.",
        "guide": "Attrition rate by department is the most critical metric. High performers leaving specific departments signal management or compensation issues.",
        "agg": "Mean",
        "color": "#E91E63"
    },
    "General / Other": {
        ...
        "segment_keywords": [],
        "metric_keywords": [],
        "description": "General dataset — auto-selected the most suitable category and metric columns for analysis.",
        "guide": "Review the auto-selected columns below. Change them if a different column better represents your grouping or measurement.",
        "agg": "Sum",
        "color": "#9D72FF"
    }
}
...
def _detect_sector(df):
    """
    Scores each sector profile against column names and sample values.
    Returns (best_sector_name, score_dict, best_sector_col, best_metric_col, suggested_agg).
    """
    all_cols = [c.lower().replace(" ", "_").replace("-", "_") for c in df.columns]
    cat_cols  = df.select_dtypes(include=['object','category']).columns.tolist()
    num_cols  = df.select_dtypes(include='number').columns.tolist()

    # Also sample string values from categorical columns for keyword matching
    sample_values = []
    for c in cat_cols[:5]:
        vals = df[c].dropna().astype(str).str.lower().unique()[:30]
        sample_values.extend(vals)
    sample_text = " ".join(all_cols + sample_values)

    scores = {}
    for sector, profile in SECTOR_PROFILES.items():
        if sector == "General / Other":
            scores[sector] = 0
            continue
        score = 0
        for kw in profile["keywords"]:
            if kw in sample_text:
                score += 2 if kw in " ".join(all_cols) else 1
        scores[sector] = score

    best_sector = max(scores, key=scores.get)
    if scores[best_sector] < 3:
        best_sector = "General / Other"

    profile = SECTOR_PROFILES[best_sector]

    # ── Smart column selection
    # Best sector col: prefer cat col whose name matches segment_keywords
    best_sector_col = cat_cols[0] if cat_cols else None
    best_sector_score = -1
    for c in cat_cols:
        cl = c.lower().replace(" ", "_")
        sc = sum(1 for kw in profile["segment_keywords"] if kw in cl)
        n_unique = df[c].nunique()
        # Ideal: 3-25 unique values, named like a segment
        quality = sc * 3 + (1 if 3 <= n_unique <= 25 else 0)
        if quality > best_sector_score:
            best_sector_score = quality
            best_sector_col = c

    # Best metric col: prefer num col whose name matches metric_keywords
    best_metric_col = num_cols[0] if num_cols else None
    best_metric_score = -1
    for c in num_cols:
        cl = c.lower().replace(" ", "_")
        mc = sum(1 for kw in profile["metric_keywords"] if kw in cl)
        if mc > best_metric_score:
            best_metric_score = mc
            best_metric_col = c

    return best_sector, scores, best_sector_col, best_metric_col, profile.get("agg", "Sum")
...
def render_sector_analysis(df):
    st.markdown("""
        <div style='padding:20px 0 10px 0;'>
            ...
            <p style='color:#B19CD9; margin:6px 0 0 0; font-size:15px;'>
                Auto-detects your dataset's industry and intelligently pre-selects the best columns for analysis.
            </p>
        </div>
    """, unsafe_allow_html=True)
    ...
    # ── Run auto-detection (cached per dataframe shape to avoid re-running on every widget interaction)
    cache_key = f"sa_detect_{len(df)}_{list(df.columns)}"
    if st.session_state.get('sa_cache_key') != cache_key:
        detected_sector, scores, auto_cat, auto_metric, auto_agg = _detect_sector(df)
        st.session_state['sa_detected']    = detected_sector
        st.session_state['sa_scores']      = scores
        st.session_state['sa_auto_cat']    = auto_cat
        st.session_state['sa_auto_metric'] = auto_metric
        st.session_state['sa_auto_agg']    = auto_agg
        st.session_state['sa_cache_key']   = cache_key
    else:
        detected_sector = st.session_state['sa_detected']
        scores          = st.session_state['sa_scores']
        auto_cat        = st.session_state['sa_auto_cat']
        auto_metric     = st.session_state['sa_auto_metric']
        auto_agg        = st.session_state['sa_auto_agg']

    profile = SECTOR_PROFILES[detected_sector]

    # ── Detection result banner
    conf_score = scores.get(detected_sector, 0)
    confidence = "High" if conf_score >= 10 else "Medium" if conf_score >= 4 else "Low"
    conf_color = {"High": "#4CAF50", "Medium": "#FFC107", "Low": "#9D72FF"}[confidence]

    st.markdown(f"""
        <div style='background:linear-gradient(135deg,rgba(40,42,54,0.95),rgba(60,63,80,0.9));
                    border-left:4px solid {profile["color"]};border-radius:12px;
                    padding:18px 22px;margin-bottom:20px;'>
            <div style='display:flex;justify-content:space-between;align-items:flex-start;flex-wrap:wrap;gap:10px;'>
                <div>
                    <p style='color:{profile["color"]};font-size:11px;font-weight:700;
                               letter-spacing:2px;text-transform:uppercase;margin:0 0 6px 0;'>
                        Auto-Detected Industry
                    </p>
                    <h3 style='color:#E0E0FF;margin:0 0 6px 0;font-size:22px;font-weight:800;'>{detected_sector}</h3>
                    <p style='color:#B19CD9;margin:0;font-size:13px;line-height:1.6;'>{profile["description"]}</p>
                </div>
                <div style='text-align:right;'>
                    <span style='background:{conf_color};color:#fff;font-size:11px;font-weight:700;
                                 padding:4px 12px;border-radius:20px;letter-spacing:1px;'>
                        {confidence} Confidence
                    </span>
                    <p style='color:#607D8B;font-size:11px;margin:6px 0 0 0;'>Detection score: {conf_score}</p>
                </div>
            </div>
            <div style='margin-top:12px;padding-top:12px;border-top:1px solid rgba(255,255,255,0.07);'>
                <p style='color:#00BCD4;font-size:13px;margin:0;'>
                    <strong>Analyst tip:</strong> {profile["guide"]}
                </p>
            </div>
        </div>
    """, unsafe_allow_html=True)

    # ── Column selection — pre-populated with smart defaults
    st.markdown("""
        <div style='background:rgba(106,90,205,0.10);border-radius:10px;
                    border-left:4px solid #9D72FF;padding:12px 18px;margin:0 0 14px 0;'>
            <p style='color:#9D72FF;font-weight:700;margin:0 0 3px 0;font-size:13px;'>
                Auto-selected columns — change if needed
            </p>
            <p style='color:#B19CD9;margin:0;font-size:12px;'>
                The system picked the most relevant columns based on your dataset. Override them freely.
            </p>
        </div>
    """, unsafe_allow_html=True)
    ...
    with sc1:
        auto_cat_idx = cat_cols.index(auto_cat) if auto_cat in cat_cols else 0
        sector_col = st.selectbox("Segment / Category column", cat_cols,
            index=auto_cat_idx, key="sa_cat",
            help="Groups your data — e.g. Region, Product Type, Loan Type, Department")
    with sc2:
        auto_metric_idx = num_cols.index(auto_metric) if auto_metric in num_cols else 0
        metric_col = st.selectbox("Metric to measure", num_cols,
            index=auto_metric_idx, key="sa_metric",
            help="The number to compare across groups — e.g. Revenue, Balance, Score")
    with sc3:
        agg_options = ["Sum", "Mean", "Count", "Median", "Max"]
        auto_agg_idx = agg_options.index(auto_agg) if auto_agg in agg_options else 0
        agg_choice = st.selectbox("Aggregation", agg_options,
            index=auto_agg_idx, key="sa_agg",
            help="Sum = total per group | Mean = average per group | Count = number of records")

    # ── Manual override + detection score expander
    with st.expander("Override detected industry / view detection scores"):
        override_sector = st.selectbox(
            "Switch to a different industry template",
            list(SECTOR_PROFILES.keys()),
            index=list(SECTOR_PROFILES.keys()).index(detected_sector),
            key="sa_override"
        )
        if override_sector != detected_sector:
            profile = SECTOR_PROFILES[override_sector]
            detected_sector = override_sector
            st.markdown(f"""
                <div style='background:rgba(40,42,54,0.7);border-radius:8px;padding:12px 16px;margin-top:8px;'>
                    <p style='color:#00BCD4;font-size:13px;margin:0;'>
                        <strong>Switched to {override_sector}:</strong> {profile["guide"]}
                    </p>
                </div>
            """, unsafe_allow_html=True)
        # Score breakdown
        st.markdown("<p style='color:#B19CD9;font-size:12px;margin:12px 0 6px 0;font-weight:700;'>Detection confidence per industry:</p>", unsafe_allow_html=True)
        sorted_scores = sorted(scores.items(), key=lambda x: -x[1])
        for sname, sscore in sorted_scores:
            if sname == "General / Other": continue
            bar_w = int(min(sscore / max(1, sorted_scores[0][1]) * 160, 160))
            color = profile["color"] if sname == detected_sector else "#555"
            st.markdown(f"""
                <div style='display:flex;align-items:center;gap:10px;margin-bottom:5px;'>
                    <span style='color:#B19CD9;font-size:12px;min-width:180px;'>{sname}</span>
                    <div style='background:#2A2A3A;border-radius:4px;height:8px;width:160px;'>
                        <div style='background:{color};border-radius:4px;height:8px;width:{bar_w}px;'></div>
                    </div>
                    <span style='color:#607D8B;font-size:11px;'>{sscore}</span>
                </div>
            """, unsafe_allow_html=True)
    ...
    with s_tabs[0]:
        _sa_n_sectors = len(grouped)
        _sa_bar_max     = max(4, min(30, _sa_n_sectors))
        _sa_bar_default = min(10, _sa_bar_max)
        _sa_bar_min     = min(3, _sa_bar_max - 1)
        top_n = st.slider("Show top N sectors", _sa_bar_min, _sa_bar_max, _sa_bar_default, key="sa_topn")
    ...
    with s_tabs[2]:
        if len(num_cols) < 2:
            ...
            _sa_radar_max     = max(3, min(12, len(grouped)))
            _sa_radar_default = min(6, _sa_radar_max)
            _sa_radar_min     = min(2, _sa_radar_max - 1)
            max_sectors = st.slider("Max sectors to show on radar", _sa_radar_min, _sa_radar_max, _sa_radar_default, key="sa_radar_n")
            ...
            if not sel_feats:
                ...
                with st.spinner("Training model..."):
                    try:
                        ...
                        mae    = mean_absolute_error(y_te, y_hat) if len(y_te) > 0 else 0.0
                        rmse   = float(np.sqrt(mean_squared_error(y_te, y_hat))) if len(y_te) > 0 else 0.0
                        r2     = r2_score(y_te, y_hat) if len(y_te) > 1 else 0.0
...
<!-- FOOTER -->
<div class="footer">
  <span>Smart Data Analysis Platform &nbsp;·&nbsp; Intelligence Report Engine</span>
